...
{{/*
Default child chart values merged with metric.kube-state-metrics.
*/}}
{{- define "childcharts.metric.kube-state-metrics.defaults" -}}
{{- .Files.Get "files/childcharts/metric/kube-state-metrics/values.yaml" -}}
{{- end -}}

{{- define "childcharts.metric.kube-state-metrics.values" -}}
{{- if and .Values (hasKey .Values "metric") -}}
{{- $defaults := include "childcharts.metric.kube-state-metrics.defaults" . | fromYaml -}}
{{- $userValues := index .Values.metric "kube-state-metrics" | default dict -}}
{{- $legacyValues := get .Values.metric "kubeStateMetrics" | default dict -}}
{{- $enabled := true -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.name" -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.fullname" -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
    {{ default (include "childcharts.metric.kube-state-metrics.fullname" .) .Values.serviceAccount.name }}
{{- else -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.namespace" -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.chart" -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.labels" }}
helm.sh/chart: {{ template "childcharts.metric.kube-state-metrics.chart" . }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
app.kubernetes.io/component: metrics
app.kubernetes.io/part-of: {{ template "childcharts.metric.kube-state-metrics.name" . }}
{{- include "childcharts.metric.kube-state-metrics.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "childcharts.metric.kube-state-metrics.selectorLabels" }}
{{- if .Values.selectorOverride }}
{{ toYaml .Values.selectorOverride }}
{{- else }}
app.kubernetes.io/name: {{ include "childcharts.metric.kube-state-metrics.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}
{{- end }}

{{/* Sets default scrape limits for servicemonitor */}}
{{- define "childcharts.metric.kube-state-metrics.servicemonitor.scrapeLimits" -}}
{{- with .sampleLimit }}
sampleLimit: {{ . }}
{{- end }}
{{- with .targetLimit }}
...
{{- define "childcharts.metric.kube-state-metrics.scrapeconfig.scrapeLimits" -}}
{{- with .sampleLimit }}
sampleLimit: {{ . }}
{{- end }}
{{- with .targetLimit }}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.imagePullSecrets" -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.image" -}}
{{- if .Values.image.sha }}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.kubeRBACProxy.image" -}}
...
*/}}
{{- define "childcharts.metric.kube-state-metrics.crsConfigMapName" -}}
  {{- if ne .Values.customResourceState.name "" }}
    {{- .Values.customResourceState.name }}
  {{- else }}
    {{- template "childcharts.metric.kube-state-metrics.fullname" . }}-customresourcestate-config
  {{- end }}
{{- end }}
