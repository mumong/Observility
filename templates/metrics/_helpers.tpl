...
{{- default .Chart.Name .Values.metric.extra.nameOverride | trunc 50 | trimSuffix "-" -}}
{{- end }}
...
{{- if .Values.metric.extra.fullnameOverride -}}
{{- .Values.metric.extra.fullnameOverride | trunc 26 | trimSuffix "-" -}}
{{- else -}}
{{- $name := default .Chart.Name .Values.metric.extra.nameOverride -}}
...
{{- if .Values.metric.prometheusOperator.fullnameOverride -}}
{{- .Values.metric.prometheusOperator.fullnameOverride | trunc 35 | trimSuffix "-" -}}
{{- else -}}
...
{{- if .Values.metric.extra.cleanPrometheusOperatorObjectNames }}
...
{{- default (printf "%s-thanos-ruler" (include "kube-prometheus-stack.name" .)) .Values.metric.others.thanosRuler.name -}}
{{- end }}
...
{{- if .Values.metric.extra.commonLabels}}
{{ toYaml .Values.metric.extra.commonLabels }}
{{- end }}
{{- end }}

{{/* Create the name of kube-prometheus-stack service account to use */}}
{{- define "kube-prometheus-stack.operator.serviceAccountName" -}}
{{- if .Values.metric.prometheusOperator.serviceAccount.create -}}
    {{ default (include "kube-prometheus-stack.operator.fullname" .) .Values.metric.prometheusOperator.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.metric.prometheusOperator.serviceAccount.name }}
{{- end -}}
{{- end -}}
...
{{- if .Values.metric.prometheusOperator.serviceAccount.create -}}
    {{ default (printf "%s-webhook" (include "kube-prometheus-stack.operator.fullname" .)) .Values.metric.prometheusOperator.admissionWebhooks.deployment.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.metric.prometheusOperator.admissionWebhooks.deployment.serviceAccount.name }}
{{- end -}}
{{- end -}}

{{/* Create the name of prometheus service account to use */}}
{{- define "kube-prometheus-stack.prometheus.serviceAccountName" -}}
{{- if .Values.metric.prometheus.serviceAccount.create -}}
    {{ default (print (include "kube-prometheus-stack.fullname" .) "-prometheus") .Values.metric.prometheus.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.metric.prometheus.serviceAccount.name }}
{{- end -}}
{{- end -}}

{{/* Create the name of alertmanager service account to use */}}
{{- define "kube-prometheus-stack.alertmanager.serviceAccountName" -}}
{{- if .Values.metric.alertmanager.serviceAccount.create -}}
    {{ default (print (include "kube-prometheus-stack.fullname" .) "-alertmanager") .Values.metric.alertmanager.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.metric.alertmanager.serviceAccount.name }}
{{- end -}}
...
{{- if .Values.metric.others.thanosRuler.serviceAccount.create -}}
    {{ default (include "kube-prometheus-stack.thanosRuler.name" .) .Values.metric.others.thanosRuler.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.metric.others.thanosRuler.serviceAccount.name }}
{{- end -}}
{{- end -}}
...
{{- define "kube-prometheus-stack.namespace" -}}
  {{- if .Values.metric.extra.namespaceOverride -}}
    {{- .Values.metric.extra.namespaceOverride -}}
  {{- else -}}
    {{- .Release.Namespace -}}
  {{- end -}}
{{- end -}}
...
{{- define "kube-prometheus-stack-grafana.namespace" -}}
  {{- if .Values.metric.grafana.namespaceOverride -}}
    {{- .Values.metric.grafana.namespaceOverride -}}
  {{- else -}}
    {{- .Release.Namespace -}}
  {{- end -}}
{{- end -}}
...
{{- define "kube-prometheus-stack-alertmanager.namespace" -}}
  {{- if .Values.metric.alertmanager.namespaceOverride -}}
    {{- .Values.metric.alertmanager.namespaceOverride -}}
  {{- else -}}
...
{{- define "kube-prometheus-stack-kube-state-metrics.name" -}}
  {{- if index .Values "metric" "kube-state-metrics" "nameOverride" -}}
    {{- index .Values "metric" "kube-state-metrics" "nameOverride" -}}
  {{- else -}}
...
{{- define "kube-prometheus-stack-kube-state-metrics.namespace" -}}
  {{- if index .Values "metric" "kube-state-metrics" "namespaceOverride" -}}
    {{- index .Values "metric" "kube-state-metrics" "namespaceOverride" -}}
  {{- else -}}
    {{- .Release.Namespace -}}
  {{- end -}}
{{- end -}}
...
{{- define "kube-prometheus-stack-prometheus-node-exporter.namespace" -}}
  {{- if index .Values "metric" "prometheus-node-exporter" "namespaceOverride" -}}
    {{- index .Values "metric" "prometheus-node-exporter" "namespaceOverride" -}}
  {{- else -}}
    {{- .Release.Namespace -}}
  {{- end -}}
{{- end -}}
...
{{- define "kube-prometheus-stack.kubeVersion" -}}
  {{- default .Capabilities.KubeVersion.Version .Values.metric.extra.kubeVersionOverride -}}
{{- end -}}
...
{{- range .Values.metric.extra.global.imagePullSecrets }}
...
{{- if .Values.metric.prometheusOperator.admissionWebhooks.deployment.enabled }}
{{ $fullname }}-webhook
...
{{- if .Values.metric.others.kubelet.serviceMonitor.https }}https{{ else }}http{{ end }}
{{- end }}
{{- define "kube-prometheus-stack.kubelet.authConfig" }}
{{- if .Values.metric.others.kubelet.serviceMonitor.https }}
tlsConfig:
  caFile: /var/run/secrets/kubernetes.io/serviceaccount/ca.crt
  insecureSkipVerify: {{ .Values.metric.others.kubelet.serviceMonitor.insecureSkipVerify }}
